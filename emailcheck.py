...
def add_and_update(bug_list, person, bug_number, date):
...
def get_bug_assignments(service):
    ...
    for bugs in bug_messages:
        ...
        bug_list = add_and_update(bug_list, person, bug_number, date)
    return bug_list

def get_closed_bugs(service):
    bug_list = []
    ...
    for bugs in bug_messages:
        ...
        bug_number = re.search('\w+-\w+', message_subject[message_subject.find('Assigned: ')+10:]).group(0)
        date = bug_message['internalDate']
        bug_list = add_and_update(bug_list, None, bug_number, date)
    return bug_list
...
def main():
    ...
    closed_bugs = get_closed_bugs(service)
    bugs_to_remove = []
    for bug_index, bug in enumerate(assigned_bugs):
        for closed_index, closed in enumerate(closed_bugs):
            if bug['number'] == closed['number']:
                #bug was closed at some point in time
                if int(bug['internalDate']) > int(closed['internalDate']):
                    #bug was reopened
                    break
                elif int(bug['internalDate']) < int(closed['internalDate']):
                    #bug was fixed and closed
                    bugs_to_remove.append(bug_index)
                    break
                else:
                    #bug assigned and closed at the same time
                    bugs_to_remove.append(bug_index)
                    break
            else:
                #no match
                pass
    for index in reversed(bugs_to_remove):
        assigned_bugs.pop(index)
    for bug in assigned_bugs:
        print(bug['number']," ",bug['assignment'])
